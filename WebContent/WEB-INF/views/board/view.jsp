			...
			<div id="board" class="board-form">
				...
				<div class="bottom">
					...
					<c:if test="${sessionScope.authUser ne null}">
						<a href="/mysite/board?a=writeform&userno=${sessionScope.authUser.no}&parentno=${vo.no}&groupno=${vo.groupNo}&orderno=${vo.orderNo}&depth=${vo.depth}">답글</a>
					</c:if>
	 			...
	 			<li>
					<table style=" margin-left: auto; margin-right: auto;" width="510" border="1">
						<tr>
							...
							<td>${vo.regDate}</td>
							<c:if test="${sessionScope.authUser.no eq vo.userNo}">
								<td><a href="/mysite/board?a=deletecomment&no=${param.no}&commentno=${vo.no}">삭제</a></td>
							</c:if>
						</tr>
						<tr>
